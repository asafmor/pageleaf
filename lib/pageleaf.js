...
import { parseUploadTarget, uploadHtml } from './upload.js';

export { PageleafError, UsageError } from './errors.js';
...
Options:
  ...
  --layout <name>   Navigation layout: ${LAYOUTS.join(', ')} (default: standard)
  --out <folder>    Output folder (default: current directory)
  --upload <[owner/]repo>  Upsert the HTML file in a GitHub repo's root on its default branch
...
Examples:
  ...
  pageleaf guide.md --upload my-docs
  pageleaf ./docs --upload my-org/my-docs

Upload requires git and gh, authenticated with "gh auth login --hostname github.com".
Omit owner to use the signed-in user's personal account. The repository must exist
and allow direct pushes. Remote files are replaced automatically; --force only
controls replacing the local output. Unchanged uploads do not create a commit.`;
...
export function embedDocument(template, markdown, theme = 'paper', layout = 'standard') {
  if (!THEMES.includes(theme)) throw new UsageError(`Unknown theme "${theme}". Choose one of: ${THEMES.join(', ')}.`);
  if (!LAYOUTS.includes(layout)) throw new UsageError(`Unknown layout "${layout}". Choose one of: ${LAYOUTS.join(', ')}.`);
  ...
  const withLayout = withTheme.replace(/(data-layout=")[^"]*(")/i, `$1${layout}$2`);
  if (withLayout === withTheme && layout !== 'standard') throw new PageleafError('The packaged HTML template has no default layout setting. Reinstall pageleaf and try again.');
  return withLayout;
}
...
export async function generateSite(input, options = {}) {
  ...
  const html = embedDocument(template, markdown, options.theme ?? 'paper', options.layout ?? 'standard');
...
export function parseCommandLine(args) {
  ...
  if (parsed.values.upload !== undefined) parseUploadTarget(parsed.values.upload);
  ...
  if (!LAYOUTS.includes(parsed.values.layout)) {
    throw new UsageError(`Unknown layout "${parsed.values.layout}". Choose one of: ${LAYOUTS.join(', ')}.`);
  }
  return parsed;
}
...
export async function run(args, io = console) {
  ...
  if (parsed.values.upload !== undefined) {
    try {
      result.upload = await uploadHtml(result.outputPath, parsed.values.upload);
      io.log(`${result.upload.changed ? 'Uploaded' : 'Already up to date:'} ${result.upload.url}`);
    } catch (error) {
      throw new PageleafError(`${error.message}\nGenerated HTML remains at "${result.outputPath}". After fixing the problem, rerun the same command with --force to retry.`);
    }
  }
